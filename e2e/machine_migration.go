...
func createCAPIMachine(ctx context.Context, cl client.Client, machineName string) *clusterv1.Machine {
	capiMachineList, err := capiframework.GetMachines(cl, ctx)
...
func cleanupMachineResources(ctx context.Context, cl client.Client, capiMachines []*clusterv1.Machine, mapiMachines []*machinev1beta1.Machine) {
	for _, m := range capiMachines {
		...
		err := capiframework.DeleteMachines(cl, ctx, capiframework.CAPINamespace, m)
		if err != nil && !apierrors.IsNotFound(err) {
			Expect(err).ToNot(HaveOccurred(), "Failed to delete CAPI Machines")
		}
	}
	...
	for _, m := range mapiMachines {
		...
		err := mapiframework.DeleteMachines(ctx, cl, m)
		if err != nil && !apierrors.IsNotFound(err) {
			Expect(err).ToNot(HaveOccurred(), "Failed to delete MAPI Machines")
		}
