...
var _ = Describe("[sig-cluster-lifecycle][OCPFeatureGate:MachineAPIMigration] Machines Migration Tests", Ordered, func() {
	...
	var _ = Describe("Mapi machine creation E2E", Ordered, func() {
		...
		It("should create a new machine from an existing one,when NO existing CAPI machine and authoritativeAPI is mapi", func() {
			...
			templateMachine := machineList.Items[0]
			By(fmt.Sprintf("Using worker machine %s as a template", templateMachine.Name))

			if templateMachine.Spec.AuthoritativeAPI == "ClusterAPI" {
				Skip("This test is for AuthoritativeAPI as MachineAPI")
			}
			...
			// Poll the machine's status until it becomes "Running" or a timeout is reached.
			err = wait.PollUntilContextTimeout(ctx, DefaultInterval, DefaultTimeout, true, func(ctx context.Context) (bool, error) {
				key := client.ObjectKey{Name: newMachine.Name, Namespace: newMachine.Namespace}
				currentMachine := &machinev1.Machine{}
				if err := cl.Get(ctx, key, currentMachine); err != nil {
					// A NotFound error is fatal, as the machine should not be deleted.
					if apierrors.IsNotFound(err) {
						GinkgoLogr.Error(err, "Machine was deleted unexpectedly, failing test.", "name", key.Name)
						return false, err
					}
					// For other transient errors, log and retry.
					GinkgoLogr.Error(err, "Failed to get machine, retryin", "name", key.Name)
					return false, nil
				}
				currentPhase := "Unknown"
				if currentMachine.Status.Phase != nil {
					currentPhase = *currentMachine.Status.Phase
				}
				GinkgoLogr.Info("Polling machine status", "name", currentMachine.Name, "phase", currentPhase)
				if currentPhase == "Running" {
					GinkgoLogr.Info("Machine entered running phase.", "machineName", currentMachine.Name)
					newMachine = currentMachine
					return true, nil
				}
				return false, nil
			})
			Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("Machine %s did not enter 'Running' phase within the timeout", newMachine.Name))
			By("Verifying the new machine has a node reference")
			Eventually(func(g Gomega) {
				Expect(newMachine.Status.NodeRef).NotTo(BeNil(), "Machine should have a node reference")
			})

			By("Verifying status of mapi machine for synchronisation done successfully")
			statusMapiMachine, err := json.Marshal(newMachine.Status)
			Expect(err).NotTo(HaveOccurred())
			statusString := string(statusMapiMachine)
			Expect(statusString).To(ContainSubstring(`"type":"Synchronized"`))
			Expect(statusString).To(ContainSubstring(`"status":"True"`))
			Expect(statusString).To(ContainSubstring("Successfully synchronized MAPI Machine to CAPI"))
			Expect(statusString).To(ContainSubstring("The AuthoritativeAPI is set to MachineAPI"))
			...
			Expect(targetMachine).NotTo(BeNil(), "Could not find the corresponding CAPI machine")
			statusCapiMachine, err := json.Marshal(targetMachine.Status)
			Expect(err).NotTo(HaveOccurred())
			statusString = string(statusCapiMachine)
			Expect(statusString).To(ContainSubstring(`"type":"Paused"`))
			Expect(statusString).To(ContainSubstring(`"status":"True"`))
			Expect(statusString).To(ContainSubstring("Machine has the cluster.x-k8s.io/paused annotation"))
		})
	})
})
