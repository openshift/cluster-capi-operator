...
var _ = Describe("[OTP][Jira:OCPCLOUD][OCPFeatureGate:ClusterAPIMachineManagementGCP][sig-cluster-lifecycle] Cluster API Secret Sync",
	Label("platform:gcp"),
	func() {
		BeforeEach(func() {
			if platform != configv1.GCPPlatformType {
				Skip("Skipping GCP-specific tests on non-GCP platform.")
			}
			if !framework.IsFeatureGateEnabled(ctx, cl, features.FeatureGateClusterAPIMachineManagementGCP) {
				Skip("Feature gate ClusterAPIMachineManagementGCP is not enabled.")
			}
		})
