...
func verifyMAPIMachineSetProviderSpec(mapiMachineSet *mapiv1beta1.MachineSet, matcher types.GomegaMatcher) {
	...
	Eventually(func() *mapiv1beta1.AWSMachineProviderConfig {
		freshMachineSet, _ := mapiframework.GetMachineSet(ctx, cl, mapiMachineSet.Name)
		return getAWSProviderSpecFromMachineSet(freshMachineSet)
	}, capiframework.WaitMedium, capiframework.RetryShort).Should(matcher)
}
