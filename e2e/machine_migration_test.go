		...
		Context("when no existing CAPI Machine with same name should allow creating the MAPI Machine with specAPI: CAPI", func() {
			...
			It("should verify that the non-authoritative MAPI Machine has an authoritative CAPI Machine mirror", func() {
				Eventually(func() error {
					newCapiMachine, err = capiframework.GetMachine(cl, machineNameCAPI, capiframework.CAPINamespace)
					return err
...
func createCAPIMachine(ctx context.Context, cl client.Client, machineName string) *clusterv1.Machine {
	...
	templateAWSMachine, err := capiframework.GetAWSMachine(cl, templateCapiMachine.Name, capiframework.CAPINamespace)
