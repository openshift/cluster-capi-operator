...
func GetAWSMachineTemplateByName(cl client.Client, name string, namespace string) (*awsv1.AWSMachineTemplate, error) {
	...
	Eventually(komega.Get(awsMachineTemplate), time.Minute, RetryShort).Should(SatisfyAny(
		Succeed(),
		WithTransform(apierrors.IsNotFound, BeTrue()),
	), "Failed to get AWSMachineTemplate %s/%s.",
		awsMachineTemplate.Namespace, awsMachineTemplate.Name)
