...
func CreateMachineSet(cl client.Client, ctx context.Context, params machineSetParams) *clusterv1.MachineSet {
...
func WaitForMachineSetsDeleted(cl client.Client, ctx context.Context, machineSets ...*clusterv1.MachineSet) {
	for _, ms := range machineSets {
		By(fmt.Sprintf("Waiting for MachineSet %q to be deleted", ms.GetName()))
		Eventually(func() bool {
			selector := ms.Spec.Selector

			machines, err := GetMachines(cl, ctx, &selector)
...
func DeleteMachineSets(cl client.Client, ctx context.Context, machineSets ...*clusterv1.MachineSet) {
	for _, ms := range machineSets {
		if ms == nil {
			continue
		}
...
func WaitForMachineSet(cl client.Client, ctx context.Context, name string, namespace string) {
	...
	Eventually(func() error {
		machines, err := GetMachinesFromMachineSet(cl, ctx, machineSet)
		if err != nil {
			return err
		}
		...
		for _, m := range running {
			node, err := GetNodeForMachine(cl, ctx, m)
			if err != nil {
				return err
			}
...
func GetMachinesFromMachineSet(cl client.Client, ctx context.Context, machineSet *clusterv1.MachineSet) ([]*clusterv1.Machine, error) {
	machines, err := GetMachines(cl, ctx)
	if err != nil {
