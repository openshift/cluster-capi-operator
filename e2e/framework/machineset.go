...
func WaitForMachineSetsDeleted(cl client.Client, machineSets ...*clusterv1.MachineSet) {
	for _, ms := range machineSets {
		...
		Eventually(func() int {
			...
			return len(machines)
		}, WaitLong, RetryMedium).Should(BeZero(), "All machines should be deleted before MachineSet")
		...
		}), WaitLong, RetryMedium).Should(WithTransform(apierrors.IsNotFound, BeTrue()), "MachineSet %s/%s should be deleted", ms.GetNamespace(), ms.GetName())
	}
}
