...
var map_MachineStatus = map[string]string{
	...
	"synchronizedAPI":        "synchronizedAPI represents the API that is currently in sync with the state of the resource. When a migration begins, this field is set to the value of the authoritativeAPI before the transition to \"Migrating\". If a migration becomes stuck and authoritativeAPI is changed back to the original value, this field is used to determine the source of the migration, allowing for a clean rollback to the previously synchronized API. An empty value indicates that the resource has not yet been part of a migration.",
}
