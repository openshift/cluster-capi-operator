...
func profile(t *testing.T, name, imageRef, profileName, manifestContent string) providerimages.ProviderImageManifests {
	...
	return test.NewProviderImageManifests(t, name).
		WithImageRef(imageRef).
		WithProfile(profileName).
		WithManifests(manifestContent).
		Build()
}
...
// forInstall creates an InstallerRevision from a ParsedRevision, failing the test on error.
