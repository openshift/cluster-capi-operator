...
func adoptTestObject(name string, annotations map[string]string) *unstructured.Unstructured {
...
func TestAdoptExistingTransformer_TransformObject(t *testing.T) {
	...
	t.Run("object without annotation returns unchanged with nil options", func(t *testing.T) {
		g := NewWithT(t)
		obj := adoptTestObject("no-annotation", nil)

		transformed, opts, err := transformer.TransformObject(ctx, obj)

		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(opts).To(BeNil())
		g.Expect(transformed).To(BeIdenticalTo(obj))
	})
	...
	t.Run("object with always: annotation stripped and CollisionProtectionNone option returned", func(t *testing.T) {
		...
		g.Expect(transformed.GetAnnotations()).NotTo(HaveKey(revisiongenerator.AdoptExistingAnnotation))

		// The original object must not be mutated.
		g.Expect(obj.GetAnnotations()).To(HaveKey(revisiongenerator.AdoptExistingAnnotation))
	})
...
func TestAdoptExistingTransformer_Validate(t *testing.T) {
	...
	t.Run("object with invalid value returns terminal error wrapping ErrInvalidAdoptExistingAnnotation", func(t *testing.T) {
		g := NewWithT(t)
