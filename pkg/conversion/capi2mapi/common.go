...
func convertCAPIMachineSetSelectorToMAPI(mapiSelector metav1.LabelSelector) metav1.LabelSelector {
	capiSelector := mapiSelector
	capiSelector.MatchLabels = convertCAPILabelsToMAPILabels(mapiSelector.MatchLabels)

	return capiSelector
}

func convertCAPILabelsToMAPILabels(capiLabels map[string]string) map[string]string {
	if len(capiLabels) == 0 || capiLabels == nil {
		return nil
	}
