...
func vsphereMachineFuzzerFuncs(codecs runtimeserializer.CodecFactory) []interface{} {
	return []interface{}{
		func(m *vspherev1.VSphereMachine, c randfill.Continue) {
			...
			m.TypeMeta.APIVersion = vspherev1.GroupVersion.String() //nolint:staticcheck // TypeMeta is embedded but we set it explicitly here
			m.TypeMeta.Kind = vsphereMachineKind                    //nolint:staticcheck // TypeMeta is embedded but we set it explicitly here
		},
		...
		func(spec *vspherev1.VSphereMachineSpec, c randfill.Continue) {
			...
			for i := range spec.Network.Devices {
				...
				spec.Network.Devices[i].DHCP4 = false
...
func vsphereMachineTemplateFuzzerFuncs(codecs runtimeserializer.CodecFactory) []interface{} {
	return []interface{}{
		func(m *vspherev1.VSphereMachineTemplate, c randfill.Continue) {
			...
			// Ensure the type meta is set correctly.
			m.TypeMeta.APIVersion = vspherev1.GroupVersion.String() //nolint:staticcheck // TypeMeta is embedded but we set it explicitly here
			m.TypeMeta.Kind = vsphereTemplateKind                   //nolint:staticcheck // TypeMeta is embedded but we set it explicitly here
		},
		func(spec *vspherev1.VSphereMachineTemplateSpec, c randfill.Continue) {
			...
			spec.Template.Spec.Thumbprint = ""
			spec.Template.Spec.Resources = vspherev1.VirtualMachineResources{}
			...
			for i := range spec.Template.Spec.Network.Devices {
				...
				spec.Template.Spec.Network.Devices[i].DHCP4 = false
