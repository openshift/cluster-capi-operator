...
func openstackMachineFuzzerFuncs(codecs runtimeserializer.CodecFactory) []interface{} {
	return []interface{}{
		func(serverGroup *openstackv1.ServerGroupParam, c fuzz.Continue) {
		...
		func(image *openstackv1.ImageParam, c fuzz.Continue) {
			// Only image names are supported
			image.ID = nil
			image.ImageRef = nil
			image.Filter = &openstackv1.ImageFilter{Name: ptr.To("custom-image")}
		},
		func(network *openstackv1.NetworkParam, c fuzz.Continue) {
			// We require either an ID or a Name, not both.
			switch c.Int31n(2) {
			case 0:
		...
		func(port *openstackv1.PortOpts, c fuzz.Continue) {
			c.FuzzNoCustom(port)
		...
		func(fixedIP *openstackv1.FixedIP, c fuzz.Continue) {
		...
		func(securityGroup *openstackv1.SecurityGroupParam, c fuzz.Continue) {
			// We require either an ID or a Name, not both.
			securityGroup.ID = ptr.To(uuid.NewString())
			securityGroup.Filter = nil
		},
		func(spec *openstackv1.OpenStackMachineSpec, c fuzz.Continue) {
			c.FuzzNoCustom(spec)

			// Fields not yet supported for conversion.
			spec.FlavorID = nil
		},
		func(m *openstackv1.OpenStackMachine, c fuzz.Continue) {
			c.FuzzNoCustom(m)
...
func openstackMachineTemplateFuzzerFuncs(codecs runtimeserializer.CodecFactory) []interface{} {
	return []interface{}{
		func(m *openstackv1.OpenStackMachineTemplate, c fuzz.Continue) {
			c.FuzzNoCustom(m)
