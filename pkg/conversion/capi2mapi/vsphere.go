...
func (m machineAndVSphereMachineAndVSphereCluster) toProviderSpec() (*mapiv1beta1.VSphereMachineProviderSpec, []string, field.ErrorList) {
	...
	allDataDisks := mergeAdditionalDisksGiB(m.vSphereMachine.Spec.DataDisks, m.vSphereMachine.Spec.AdditionalDisksGiB)
	...
	mapiProviderConfig := buildMAPIVSphereProviderConfig(
		m.vSphereMachine.Spec,
		ptr.Deref(m.machine.Spec.Bootstrap.DataSecretName, ""),
		mapiCloneMode,
		mapiNetworkSpec,
		mapiDataDisks,
	)
...
// mergeAdditionalDisksGiB converts the deprecated AdditionalDisksGiB field to VSphereDisk format
// and appends them to the existing DataDisks.
func mergeAdditionalDisksGiB(dataDisks []vspherev1.VSphereDisk, additionalDisksGiB []int32) []vspherev1.VSphereDisk {
	for i, sizeGiB := range additionalDisksGiB {
		dataDisks = append(dataDisks, vspherev1.VSphereDisk{
...
// buildMAPIVSphereProviderConfig assembles the MAPI VSphereMachineProviderSpec from converted sub-fields.
//
// The following CAPV VSphereMachine fields are NOT converted into the provider config:
...
//   - Thumbprint - Not required, TLS validation handled by cluster-level configuration.
//   - StoragePolicyName - Not supported in MAPI VSphereMachineProviderSpec.
//   - Resources - CPU/memory reservations, limits, and shares not supported in MAPI.
//   - AdditionalDisksGiB - Deprecated in CAPV, merged into DataDisks by mergeAdditionalDisksGiB.
//   - CustomVMXKeys - Not supported in MAPI VSphereMachineProviderSpec.
//   - PciDevices - Not supported in MAPI VSphereMachineProviderSpec.
//   - OS - Not supported in MAPI VSphereMachineProviderSpec.
//   - HardwareVersion - Not supported in MAPI VSphereMachineProviderSpec.
func buildMAPIVSphereProviderConfig(
	spec vspherev1.VSphereMachineSpec,
	bootstrapDataSecretName string,
	cloneMode mapiv1beta1.CloneMode,
	networkSpec mapiv1beta1.NetworkSpec,
	dataDisks []mapiv1beta1.VSphereDisk,
) *mapiv1beta1.VSphereMachineProviderSpec {
	config := &mapiv1beta1.VSphereMachineProviderSpec{
		...
		NumCoresPerSocket: spec.NumCoresPerSocket,
	...
	if bootstrapDataSecretName != "" {
		config.UserDataSecret = &corev1.LocalObjectReference{
...
func (m machineAndVSphereMachineAndVSphereCluster) buildAdditionalMetadata() (map[string]string, map[string]string) {
	...
	additionalMachineAPIMetadataLabels = m.buildMachineAPIMetadataLabels(additionalMachineAPIMetadataLabels)

	additionalMachineAPIMetadataAnnotations = map[string]string{
		consts.MAPIMachineMetadataAnnotationInstanceState: m.getInstanceState(),
	}

	return additionalMachineAPIMetadataLabels, additionalMachineAPIMetadataAnnotations
}

// buildMachineAPIMetadataLabels adds vSphere-specific MAPI metadata labels (instance type, region).
func (m machineAndVSphereMachineAndVSphereCluster) buildMachineAPIMetadataLabels(labels map[string]string) map[string]string {
	...
	if m.vSphereMachine.Spec.Template != "" {
		labels[consts.MAPIMachineMetadataLabelInstanceType] = m.vSphereMachine.Spec.Template
	}

	if m.vSphereMachine.Spec.Datacenter != "" {
		labels[consts.MAPIMachineMetadataLabelRegion] = m.vSphereMachine.Spec.Datacenter
	}
