...
var _ = Describe("mapi2capi VSphere conversion", func() {
	...
	var _ = DescribeTable("mapi2capi VSphere convert MAPI Machine",
		...
		Entry("With custom credentials secret", vsphereMAPI2CAPIConversionInput{
			...
			infra: infra,
			expectedErrors: []string{
				"spec.providerSpec.value.credentialsSecret: Invalid value: \"custom-vsphere-credentials\": credentialsSecret does not match the default of \"vsphere-cloud-credentials\", credentials must be configured at the cluster level",
			},
			expectedWarnings: []string{},
		}),
		...
		Entry("With VMGroup in workspace", vsphereMAPI2CAPIConversionInput{
			machineBuilder: machinebuilder.Machine().WithProviderSpec(mapiv1beta1.ProviderSpec{
				Value: mustConvertVSphereProviderSpecToRawExtension(&mapiv1beta1.VSphereMachineProviderSpec{
					Template: "test-template",
					Workspace: &mapiv1beta1.Workspace{
			...
			infra: infra,
			expectedErrors: []string{
				"spec.providerSpec.value.workspace.vmGroup: Invalid value: \"vm-group-1\": vmGroup is not supported in Cluster API",
			},
			expectedWarnings: []string{},
		}),
	)
