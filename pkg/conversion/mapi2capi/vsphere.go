...
func (v *vSphereMachineSetAndInfra) ToMachineSetAndMachineTemplate() (*clusterv1.MachineSet, client.Object, []string, error) {
	...
	// vSphere stores FailureDomain in template labels (machine.openshift.io/zone), not in ProviderSpec.
	// Read it directly from the MachineSet template since the synthetic machine has no ObjectMeta.
	if zone, ok := v.machineSet.Spec.Template.ObjectMeta.Labels[consts.MAPIMachineMetadataLabelZone]; ok && zone != "" { //nolint:staticcheck // ObjectMeta is a field, not embedded
		capiMachineSet.Spec.Template.Spec.FailureDomain = zone
	}
...
func (v *vSphereMachineAndInfra) toVSphereMachine(providerSpec mapiv1beta1.VSphereMachineProviderSpec) (*vspherev1.VSphereMachine, []string, field.ErrorList) {
	...
	spec, warnings, specErrs := buildVSphereMachineSpec(fldPath, providerSpec)
	if len(specErrs) > 0 {
		errs = append(errs, specErrs...)
	}

	errs = append(errs, validateMAPIVSphereUnsupportedFields(fldPath, providerSpec)...)
...
// buildVSphereMachineSpec constructs the CAPV VSphereMachineSpec from a MAPI VSphereMachineProviderSpec.
// See inline comments for details on which fields are converted, set elsewhere, or unsupported.
func buildVSphereMachineSpec(fldPath *field.Path, providerSpec mapiv1beta1.VSphereMachineProviderSpec) (vspherev1.VSphereMachineSpec, []string, field.ErrorList) {
	var errs field.ErrorList
	...
	capiCloneMode, cloneModeWarnings := convertVSphereCloneModeMAPIToCAPI(providerSpec.CloneMode)
	warnings = append(warnings, cloneModeWarnings...)
	...
	return spec, warnings, errs
}

// validateMAPIVSphereUnsupportedFields checks for MAPI VSphereMachineProviderSpec fields
// that cannot be converted to CAPV and returns errors for non-default values.
//
// Unused fields that are intentionally ignored:
...
func validateMAPIVSphereUnsupportedFields(fldPath *field.Path, providerSpec mapiv1beta1.VSphereMachineProviderSpec) field.ErrorList {
	var errs field.ErrorList
...
func convertVSphereNetworkSpecMAPIToCAPI(fldPath *field.Path, mapiNetwork mapiv1beta1.NetworkSpec) (vspherev1.NetworkSpec, []string, field.ErrorList) { //nolint:unparam
	...
	for i, device := range mapiNetwork.Devices {
		...
		if len(device.AddressesFromPools) > 0 {
			pools, poolWarnings := convertMAPIAddressesFromPoolsToCAPI(device.AddressesFromPools)
			devices[i].AddressesFromPools = pools
...
// convertMAPIAddressesFromPoolsToCAPI converts MAPI AddressesFromPool entries to CAPI TypedLocalObjectReferences,
// mapping plural resource names to singular Kind names using the ipamResourceToKind lookup table.
func convertMAPIAddressesFromPoolsToCAPI(mapiPools []mapiv1beta1.AddressesFromPool) ([]corev1.TypedLocalObjectReference, []string) {
	var warnings []string

	capiPools := make([]corev1.TypedLocalObjectReference, len(mapiPools))
	for i, pool := range mapiPools {
		kind, poolWarnings := resolveIPAMKindFromResource(pool.Group, pool.Resource)
		warnings = append(warnings, poolWarnings...)

		capiPools[i] = corev1.TypedLocalObjectReference{
			APIGroup: &pool.Group,
			Kind:     kind,
			Name:     pool.Name,
		}
	...
	return capiPools, warnings
}

// resolveIPAMKindFromResource maps an IPAM API group and resource name to the corresponding Kind name.
func resolveIPAMKindFromResource(group, resource string) (string, []string) {
	if group == "" {
		return resource, nil
	}

	groupMappings, ok := ipamResourceToKind[group]
	if !ok {
	...
	if mappedKind, ok := groupMappings[resource]; ok {
		return mappedKind, nil
	}
...
func convertVSphereCloneModeMAPIToCAPI(mapiMode mapiv1beta1.CloneMode) (vspherev1.CloneMode, []string) {
	switch mapiMode {
	...
	case "":
		...
		return vspherev1.FullClone, nil
	default:
		return vspherev1.FullClone, []string{
			fmt.Sprintf("unknown clone mode %q, defaulting to FullClone", mapiMode),
		}
	}
}
