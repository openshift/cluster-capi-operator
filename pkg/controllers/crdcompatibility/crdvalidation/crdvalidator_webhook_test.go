...
func Test_crdValidator_validateCreateOrUpdate(t *testing.T) { //nolint:funlen
	...
	tests := []struct {
		...
		{
			name: "Should permit an incompatible CRD with excluded fields and warnings for CRDAdmitAction set to Warn",
			obj:  testCRDWorking.DeepCopy(),
			requirements: []client.Object{
				func() *apiextensionsv1alpha1.CompatibilityRequirement {
					r := test.GenerateTestCompatibilityRequirement(testCRDWorking.DeepCopy())
		...
		{
			name: "Should permit an incompatible CRD with multiple excluded fields and warnings for CRDAdmitAction set to Warn",
			obj:  testCRDWorking.DeepCopy(),
			requirements: []client.Object{
				func() *apiextensionsv1alpha1.CompatibilityRequirement {
					r := test.GenerateTestCompatibilityRequirement(testCRDWorking.DeepCopy())
		...
		{
			name: "Should reject a CRD when a CopmatibliityRequirement has an invalid CRDAdmitAction",
			obj:  testCRDWorking.DeepCopy(),
			requirements: []client.Object{
				func() *apiextensionsv1alpha1.CompatibilityRequirement {
					r := test.GenerateTestCompatibilityRequirement(testCRDWorking.DeepCopy())
...
func Test_pruneExludedFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantCRD: func() *apiextensionsv1.CustomResourceDefinition {
				...
				metadataSchema.Required = []string{"zone"}
				return crd
			}(),
		},
		{
		...
		},
		{
			...
			wantErr: MatchError("failed to prune excluded fields: path ^.spec.not-found not found in schema, path ^.spec is missing child not-found"),
		},
		{
			...
			wantErr: MatchError("failed to prune excluded fields: path ^.spec.tags.not-found not found in schema, path ^.spec.tags is missing child not-found"),
		},
		{
			...
			wantErr: MatchError("failed to prune excluded fields: path ^.spec.name.not-found not found in schema, path ^.spec.name is not an object"),
		},
		{
			...
			wantErr: MatchError("failed to prune excluded fields: path ^.spec.tags.not-found not found in schema, path ^.spec.tags is an array but does not have an items schema"),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			gotCRD, gotErr := pruneExludedFields(tt.crd, tt.excludedFields)
			if tt.wantErr != nil {
				g.Expect(gotErr).To(tt.wantErr)
				return
			} else {
...
func complexCRDSchema() *apiextensionsv1.CustomResourceDefinition {
	...
	statusSchema := test.NewObjectSchema().
		...
		WithStringEnum("Pending", "Running", "Succeeded", "Failed").
