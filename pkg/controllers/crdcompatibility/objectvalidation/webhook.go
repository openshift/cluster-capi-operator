...
func (v *validator) ValidateCreate(ctx context.Context, compatibilityRequirementName string, obj *unstructured.Unstructured) (warnings admission.Warnings, err error) {
	validator, celValidator, err := v.createSchemaValidator(ctx, compatibilityRequirementName, obj.GroupVersionKind().Version)
	if err != nil {
	...
	return warnings, fmt.Errorf("validation failed: %w", res.AsError())
}

// ValidateUpdate validates the update of an object.
func (v *validator) ValidateUpdate(ctx context.Context, compatibilityRequirementName string, oldObj, obj *unstructured.Unstructured) (warnings admission.Warnings, err error) {
	validator, celValidator, err := v.createSchemaValidator(ctx, compatibilityRequirementName, obj.GroupVersionKind().Version)
	if err != nil {
		return nil, fmt.Errorf("failed to validate: %w", err)
	}
...
func (v *validator) createSchemaValidator(ctx context.Context, compatibilityRequirementName string, version string) (apiextensionsvalidation.SchemaValidator, *cel.Validator, error) {
	...
	if err := v.client.Get(ctx, client.ObjectKey{Name: compatibilityRequirementName}, compatibilityRequirement); err != nil {
		return nil, nil, fmt.Errorf("failed to get CompatibilityRequirement %q: %w", compatibilityRequirementName, err)
	}
	...
	if err := yaml.Unmarshal([]byte(compatibilityRequirement.Spec.CompatibilitySchema.CustomResourceDefinition.Data), &compatibilityCRD); err != nil {
		return nil, nil, fmt.Errorf("failed to parse compatibilityCRD for CompatibilityRequirement %q: %w", compatibilityRequirement.Name, err)
	}

	if compatibilityCRD.APIVersion != "apiextensions.k8s.io/v1" || compatibilityCRD.Kind != "CustomResourceDefinition" {
		return nil, nil, fmt.Errorf("%w: expected APIVersion to be apiextensions.k8s.io/v1 and Kind to be CustomResourceDefinition, got %s/%s", errObjectValidator, compatibilityCRD.APIVersion, compatibilityCRD.Kind)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("the server could not properly serve the CR schema: %w", err)
	} else if validationSchema == nil {
		return nil, nil, fmt.Errorf("%w: validationSchema can't be nil", errObjectValidator)
	}
	...
	if validationSchema != nil {
		...
		if err := apiextensionsv1.Convert_v1_CustomResourceValidation_To_apiextensions_CustomResourceValidation(validationSchema, internalValidationSchema, nil); err != nil {
			return nil, nil, fmt.Errorf("failed to convert CRD validation to internal version: %w", err)
		}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create a SchemaValidator: %w", err)
	}
