...
const (
	...
	// Requeue delays (tunable; avoid magic numbers).
	requeueUnknownAuthoritativeAPI    = 2 * time.Second
	requeueUnexpectedAuthoritativeAPI = 2 * time.Second
...
func (r *MachineSyncReconciler) Reconcile(ctx context.Context, req reconcile.Request) (ctrl.Result, error) {
	...
	case authoritativeAPI == "":
		...
		return ctrl.Result{RequeueAfter: requeueUnknownAuthoritativeAPI}, nil
	default:
		logger.Info("Machine status.authoritativeAPI has unexpected value", "AuthoritativeAPI", mapiMachine.Status.AuthoritativeAPI)
		return ctrl.Result{RequeueAfter: requeueUnexpectedAuthoritativeAPI}, nil
	}
}
