	...
	Context("when the MAPI machine does not exist and the CAPI machine does", func() {
		Context("and there is no CAPI machineSet owning the machine", func() {
			BeforeEach(func() {
				...
				capiMachine = capiMachineBuilder.WithName("test-machine-no-machineset").WithInfrastructureRef(corev1.ObjectReference{
					Kind:      capaMachine.Kind,
					Name:      capaMachine.GetName(),
					Namespace: capaMachine.GetNamespace(),
				}).Build()
	...
	Context("validating admission policy tests", func() {
		...
		FContext("Validate creation of CAPI machine", func() {
			...
			Context("when MAPI machine has status.authoritativeAPI=MachineAPI", func() {
				...
				It("allows creating CAPI machine with Paused condition", func() {
					By("Creating CAPI machine with Paused condition in status")
					newCapiMachine := clusterv1resourcebuilder.Machine().
						WithName("validation-machine").
						WithNamespace(capiNamespace.Name).
						WithConditions(clusterv1.Conditions{{
							Type:   "Paused",
			...
			Context("when authoritativeAPI is in other states", func() {

				It("allows creation when authoritativeAPI is Migrating", func() {
					By("Creating MAPI machine with authoritativeAPI=Migrating")
					mapiMachine = mapiMachineBuilder.WithName("validation-machine").Build()
					Eventually(k8sClient.Create(ctx, mapiMachine)).Should(Succeed())
					...
					// Do we want to block here?
					By("Creating CAPI machine should succeed")
					newCapiMachine := clusterv1resourcebuilder.Machine().
						WithName("validation-machine").
						WithNamespace(capiNamespace.Name).
						Build()
