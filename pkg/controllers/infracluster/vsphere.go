...
var (
	errObtainingVSphereServerAddr             = errors.New("error obtaining VSphere server address")
	errUnableToGetVSphereMAPIProviderSpec     = errors.New("unable to get VSphere MAPI ProviderSpec")
	errUnableToFindCredsSecret                = errors.New("unable to get the VSphere credentials secret")
...
func (r *InfraClusterController) ensureVSphereCluster(ctx context.Context, log logr.Logger) (client.Object, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %w", errObtainingVSphereServerAddr, err)
	}
...
func (r *InfraClusterController) getVSphereCredentials(ctx context.Context, vsphereServerAddr string) (string, string, error) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{
		...
		return "", "", fmt.Errorf("%w %s/%s: %w", errUnableToFindCredsSecret, kubeSystemNamespace, vSphereCredentialsName, err)
	}
...
func (r *InfraClusterController) getVSphereServerAddr(ctx context.Context) (string, error) {
	if r.Infra.Spec.PlatformSpec.VSphere == nil || len(r.Infra.Spec.PlatformSpec.VSphere.VCenters) == 0 {
		...
		if err != nil {
			return "", fmt.Errorf("%w: %w", errUnableToGetVSphereMAPIProviderSpec, err)
		}
