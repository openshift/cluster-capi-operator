...
func (r *InfraClusterController) getRawMAPIProviderSpec(ctx context.Context, cl client.Client) ([]byte, error) {
	...
	if cpms == nil {
		...
		// Use the youngest machine's provider spec.
		youngest := machineList.Items[0]
		for i := 1; i < len(machineList.Items); i++ {
			if machineList.Items[i].CreationTimestamp.After(youngest.CreationTimestamp.Time) {
				youngest = machineList.Items[i]
			}
		}

		return youngest.Spec.ProviderSpec.Value.Raw, nil
	}
