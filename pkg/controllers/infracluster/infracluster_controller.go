...
func (r *InfraClusterController) ensureVSphereCluster(ctx context.Context, log logr.Logger) (client.Object, error) {
	...
	if err := r.ensureVSphereSecret(ctx); err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("error obtaining VSphere server address")
	}
...
func (r *InfraClusterController) getVSphereServerAddr(ctx context.Context) (string, error) {
	if r.Infra.Spec.PlatformSpec.VSphere == nil || len(r.Infra.Spec.PlatformSpec.VSphere.VCenters) == 0 {
		...
		if err != nil || machineSpec == nil {
			return "", fmt.Errorf("unable to get VSphere MAPI ProviderSpec")
		}

		return machineSpec.Workspace.Server, nil
	}

	// TODO: here we just take the first VCenter as the one we want CAPV to interact with.
...
// getVSphereMAPIProviderSpec returns a VSphere Machine ProviderSpec from the ones present in the cluster.
func getVSphereMAPIProviderSpec(ctx context.Context, cl client.Client) (*mapiv1.VSphereMachineProviderSpec, error) {
...
func (r *InfraClusterController) ensureVSphereSecret(ctx context.Context) error {
	...
	// The CAPI VSphere credentials secret does not exist. Create it.
	vsphereServerAddr, err := r.getVSphereServerAddr(ctx)
	if err != nil {
		return fmt.Errorf("unable to get VSphere server address: %w", err)
	}
...
func (r *InfraClusterController) getVSphereCredentials(ctx context.Context, vsphereServerAddr string) (string, string, error) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{
		...
		return "", "", fmt.Errorf("unable to get the VSphere credentials secret %s/%s",
			kubeSystemNamespace, vSphereCredentialsName)
	}

	username, ok := vSphereCredentialsSecret.Data[fmt.Sprintf("%s.username", vsphereServerAddr)]
	if !ok {
		return "", "", fmt.Errorf("unable to find username in the VSphere credentials secret %s/%s",
			kubeSystemNamespace, vSphereCredentialsName)
	}

	password, ok := vSphereCredentialsSecret.Data[fmt.Sprintf("%s.password", vsphereServerAddr)]
	if !ok {
		return "", "", fmt.Errorf("unable to find password in the VSphere credentials secret %s/%s",
			kubeSystemNamespace, vSphereCredentialsName)
	}
