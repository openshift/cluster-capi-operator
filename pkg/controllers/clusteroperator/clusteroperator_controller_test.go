...
var _ = Describe("ClusterOperator controller", func() {
	...
	Context("With a supported platform", func() {
		JustBeforeEach(func() {
			By("Setting up the controllers conditions")
			co := configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build()
			patchBase := co.DeepCopy()
			co.Status.Conditions = []configv1.ClusterOperatorStatusCondition{
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.CoreClusterControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.CoreClusterControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSyncControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSyncControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
				...
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.SecretSyncControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.SecretSyncControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSetSyncControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSetSyncControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.InfraClusterControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.InfraClusterControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
			}
			Expect(cl.Status().Patch(ctx, co, client.MergeFrom(patchBase)))
		...
		It("should update the ClusterOperator status with an 'unsupported' message", func() {
			Eventually(komega.Object(configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build())).
				Should(HaveField("Status.Conditions", SatisfyAll(
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorAvailable)), HaveField("Status", Equal(configv1.ConditionTrue)), HaveField("Message", Equal("Cluster API is not yet implemented on this platform")))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorProgressing)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorDegraded)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorUpgradeable)), HaveField("Status", Equal(configv1.ConditionTrue)))),
				)), "should match the expected ClusterOperator status conditions")
	...
	Context("With a supported platform and operator status conditions needs aggregating", func() {
		It("should aggregate the top level operator conditions when all is as expected", func() {
			By("Setting up the controllers conditions")
			co := configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build()
			patchBase := co.DeepCopy()
			co.Status.Conditions = []configv1.ClusterOperatorStatusCondition{
			...
			Expect(cl.Status().Patch(ctx, co, client.MergeFrom(patchBase))).To(Succeed(), "should be able to patch the ClusterOperator")

			By("Running the reconciliation")
			_, err := (&ClusterOperatorController{
			...
			}).Reconcile(ctx, reconcile.Request{NamespacedName: types.NamespacedName{Name: capiClusterOperator.Name}})
			Expect(err).ToNot(HaveOccurred(), "should be able to reconcile the cluster-api ClusterOperator without erroring")

			By("Running checking the conditions are as expected")
			Eventually(komega.Object(configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build())).
				Should(HaveField("Status.Conditions", SatisfyAll(
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorAvailable)), HaveField("Status", Equal(configv1.ConditionTrue)),
						HaveField("Message", Equal(fmt.Sprintf("Cluster CAPI Operator is available at %s", desiredOperatorReleaseVersion))))),
		...
		It("should aggregate the top level operator conditions with degraded=true when some controller conditions are degraded", func() {
			By("Setting up the controllers conditions, one degraded")
			startingCO := configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build()
			patchBase := startingCO.DeepCopy()
			startingCO.Status.Conditions = []configv1.ClusterOperatorStatusCondition{
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.CoreClusterControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.CoreClusterControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSyncControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.MachineSyncControllerDegradedCondition, Status: configv1.ConditionFalse, Reason: operatorstatus.ReasonAsExpected},
			...
			Expect(cl.Status().Patch(ctx, startingCO, client.MergeFrom(patchBase))).To(Succeed())
			...
			Eventually(co).
				Should(HaveField("Status.Conditions", SatisfyAll(
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorAvailable)), HaveField("Status", Equal(configv1.ConditionTrue)),
						HaveField("Message", Equal(fmt.Sprintf("Cluster CAPI Operator is available at %s", desiredOperatorReleaseVersion))))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorProgressing)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					// This should be upgradeable=false due to CapiInstaller controller being degraded.
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorUpgradeable)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					// This should be degraded=true due to CapiInstaller controller being degraded.
					ContainElement(And(
						HaveField("Type", Equal(configv1.OperatorDegraded)),
						HaveField("Status", Equal(configv1.ConditionTrue)),
						HaveField("Reason", Equal(operatorstatus.ReasonControllersDegraded)),
						HaveField("Message", Equal(fmt.Sprintf("The following controllers degraded conditions are not as expected: %s=True", operatorstatus.CapiInstallerControllerDegradedCondition))),
					)),
				)), "should match the expected ClusterOperator status conditions")
		})

		It("should aggregate the top level operator conditions with available=false when some controller conditions are not available", func() {
			By("Setting up the controllers conditions, one not available")
			startingCO := configv1resourcebuilder.ClusterOperator().WithName(controllers.ClusterOperatorName).Build()
			patchBase := startingCO.DeepCopy()
			startingCO.Status.Conditions = []configv1.ClusterOperatorStatusCondition{
				{LastTransitionTime: metav1.Now(), Type: operatorstatus.CoreClusterControllerAvailableCondition, Status: configv1.ConditionTrue, Reason: operatorstatus.ReasonAsExpected},
			...
			Eventually(co).
				Should(HaveField("Status.Conditions", SatisfyAll(
					// This should be available=false due to Secret Sync controller being not available.
					ContainElement(And(
						HaveField("Type", Equal(configv1.OperatorAvailable)),
						HaveField("Status", Equal(configv1.ConditionFalse)),
						HaveField("Reason", Equal(operatorstatus.ReasonControllersNotAvailable)),
						HaveField("Message", Equal(fmt.Sprintf("The following controllers available conditions are not as expected: %s=False", operatorstatus.SecretSyncControllerAvailableCondition))),
					)),
					// This should be upgradeable=false due to Secret Sync controller being not available.
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorUpgradeable)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorProgressing)), HaveField("Status", Equal(configv1.ConditionFalse)))),
					ContainElement(And(HaveField("Type", Equal(configv1.OperatorDegraded)), HaveField("Status", Equal(configv1.ConditionFalse)))),
				)), "should match the expected ClusterOperator status conditions")
