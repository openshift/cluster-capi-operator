...
func (c *InstallerController) Reconcile(ctx context.Context, _ ctrl.Request) (ctrl.Result, error) {
	...
	reconcileResult, status := c.reconcile(ctx, log)
	reconcileResult = reconcileResult.WithStatus(status)
...
func (c *InstallerController) reconcile(ctx context.Context, log logr.Logger) (operatorstatus.ReconcileResult, *configv1apply.ClusterOperatorStatusApplyConfiguration) {
	...
	if err := c.client.Get(ctx, client.ObjectKey{Name: clusterAPIName}, clusterAPI); err != nil {
		if apierrors.IsNotFound(err) {
			return opresult.WaitingOnExternal("ClusterAPI"), nil
		}

		return opresult.Error(fmt.Errorf("fetching ClusterAPI: %w", err)), nil
	}

	if len(clusterAPI.Status.Revisions) == 0 {
		return opresult.WaitingOnExternal("ClusterAPI revisions"), nil
	}
	...
	status := revisionReconciler.relatedObjectsStatusApplyConfiguration()
	...
	if err := c.updateWatches(ctx, log, clusterAPI, revisionReconciler.gvks); err != nil {
		return opresult.Error(err), status
	}

	if reconciledRevision != nil {
		return c.success(ctx, log, clusterAPI, *reconciledRevision, errs), status
	}

	if len(errs) > 0 {
		return c.error(errs), status
	}
...
func (c *InstallerController) error(errs []error) operatorstatus.ReconcileResult {
	...
	for _, err := range errs {
		if errors.Is(err, reconcile.TerminalError(nil)) {
			...
			terminalErrors = append(terminalErrors, errors.Unwrap(err))
		} else {
	...
	if len(nonTerminalErrors) > 0 {
		nonTerminalErrors = append(nonTerminalErrors, terminalErrors...)
