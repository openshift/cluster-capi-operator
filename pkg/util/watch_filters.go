...
// ResolveCAPIMachineSetFromInfraMachineTemplate resolves synchronized machineSet
// 1. takes a client.Object (expecting a CAPI InfrastructureMachineTemplate)
// and checks if it has a machine.openshift.io/cluster-api-machine-set label
// 2. If it is, returns a reconcile.Request for the MachineSet in the MAPI namespace, so we
...
func ResolveCAPIMachineSetFromInfraMachineTemplate(namespace string) func(context.Context, client.Object) []reconcile.Request {
	return func(ctx context.Context, obj client.Object) []reconcile.Request {
		...
		if ok {
			klog.V(4).Info("Object has machine.openshift.io/cluster-api-machine-set label, enqueueing request",
