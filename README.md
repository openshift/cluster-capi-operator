...
- Management kubeconfig Secret - `<InfrastructureName>-kubeconfig` in `openshift-cluster-api`, consumed by CAPI components running in-cluster. The revision installer owns this Secret and stores only the API endpoint plus projected ServiceAccount `tokenFile` and CA paths; it is not a workstation-usable kubeconfig.
